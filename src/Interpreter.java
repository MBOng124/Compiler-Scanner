...
public class Interpreter<T> extends javaBaseVisitor<T>{
    ...
    @Override
    public T visitAssignment(javaParser.AssignmentContext ctx) {
        String text = ctx.getText();
        if(text.contains("+")||text.contains("*") || text.contains("-") || text.contains("/")) {
            System.out.println(evalMath(ctx.expression()));
        }
        return visitChildren(ctx);
    }
    ...
    @Override public T visitIfThenStatement(javaParser.IfThenStatementContext ctx) {
        for (ParseTree ins: ctx.children) {
            visit(ins);
        }
        return visitChildren(ctx);
    }
    ...
    @Override
    public T visitIfThenElseStatement(javaParser.IfThenElseStatementContext ctx) {
        return visitChildren(ctx);
    }

    public BigDecimal evalMath(javaParser.ExpressionContext ctx){
        BigDecimal result;
        String text = ctx.getText();
